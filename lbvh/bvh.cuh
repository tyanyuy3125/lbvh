...
template <typename Real, unsigned int dim, typename Object, bool IsConst> struct basic_device_bvh;
template <typename Real, unsigned int dim, typename Object> struct basic_device_bvh<Real, dim, Object, false> {
...
template <typename Real, unsigned int dim, typename Object> struct basic_device_bvh<Real, dim, Object, true> {
    using real_type = Real;
    using aabb_type = aabb<real_type, dim>;
    using node_type = detail::node;
    using index_type = std::uint32_t;
...
__device__ inline uint2 determine_range(UInt const *node_code, const unsigned int num_leaves, unsigned int idx) {
    ...
    if (0 <= i_tmp && i_tmp < num_leaves) { delta = common_upper_bits(self_code, node_code[i_tmp]); }
...
__device__ inline unsigned int find_split(
    UInt const *node_code, const unsigned int num_leaves, const unsigned int first, const unsigned int last
) noexcept {
...
void construct_internal_nodes(
    ...
    thrust::for_each(
        ...
        [self, node_code, num_objects] __device__(const unsigned int idx) {
            ...
            if (thrust::min(ij.x, ij.y) == gamma) { self.nodes[idx].left_idx += num_objects - 1; }
            if (thrust::max(ij.x, ij.y) == gamma + 1) { self.nodes[idx].right_idx += num_objects - 1; }
...
template <typename Real, unsigned int dim, typename Object> struct default_morton_code_calculator {
...
template <
    typename Real, unsigned int dim, typename Object, typename AABBGetter,
...
class bvh {
  ...
  public:
    ...
    void construct() {
        ...
        thrust::transform(
            indices.begin(), indices.end(), this->nodes_.begin() + num_internal_nodes,
