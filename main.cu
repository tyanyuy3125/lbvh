// These test cases are not maintained, and some modifications are needed.
...
struct aabb_getter {
    __device__ lbvh::aabb<float> operator()(const float4 f) const noexcept {
...
struct distance_calculator {
    __device__ float operator()(const float4 point, const float4 object) const noexcept {
        return (point.x - object.x) * (point.x - object.x) + (point.y - object.y) * (point.y - object.y) +
...
int main() {
    ...
    thrust::for_each(
        thrust::device, thrust::make_counting_iterator<std::size_t>(0), thrust::make_counting_iterator<std::size_t>(N),
        [bvh_dev] __device__(std::size_t idx) {
            unsigned int buffer[10];
            const auto self = bvh_dev.objects[idx];
            const float dr = 0.1f;
            for (std::size_t i = 1; i < 10; ++i) {
                for (unsigned int j = 0; j < 10; ++j) { buffer[j] = 0xFFFFFFFF; }
                const float r = dr * i;
                ...
                const auto num_found = lbvh::query_device(bvh_dev, lbvh::overlaps(query_box), buffer, 10);
    ...
    thrust::for_each(
        ...
        [bvh_dev] __device__(const unsigned int idx) {
            const auto self = bvh_dev.objects[idx];
            const auto nest = lbvh::query_device(bvh_dev, lbvh::nearest(self), distance_calculator());
    ...
    thrust::for_each(random_points.begin(), random_points.end(), [bvh_dev] __device__(const float4 pos) {
    ...
    std::cout << "testing query_host:overlap ...\n";
    {
        for (std::size_t i = 0; i < 10; ++i) {
            ...
            for (unsigned int cnt = 1; cnt < 10; ++cnt) {
                ...
                const auto num_found = lbvh::query_host(bvh, lbvh::overlaps(query_box), std::back_inserter(buffer));
